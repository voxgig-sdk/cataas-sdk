...
Learn more about CATAAS API at [cataas.com](https://cataas.com).
...
This SDK exposes the API as a small set of **semantic entities** — Cat and Tag — that you
...
const items = await client.Cat().list()
```
...
```go
client := sdk.Test()
result, err := client.Cat(nil).List(
    nil, nil,
)
```
...
$cats = $client->Cat()->list();
print_r(array_map(fn($item) => $item->data_get(), $cats));

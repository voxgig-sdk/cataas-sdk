...
//    only - a typed CataasContext there would make every ctx
...
final class CataasOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new CataasContext($clean, null);
    ...
    public static function err_from_map($m): ?CataasError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new CataasError(is_string($code) ? $code : '', $msg);
    }

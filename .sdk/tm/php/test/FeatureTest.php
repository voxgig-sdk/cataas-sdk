...
class FtCtrl extends CataasControl
{
...
class FtClient
{
    ...
    private CataasUtility $utility;

    public function __construct(CataasUtility $utility, array $options)
    {
    ...
    public function get_utility(): CataasUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CataasContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CataasContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CataasConfig::make_feature($name);
    ...
    private static function buildUrl(CataasSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CataasError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CataasResponse($fetched);
        }
    ...
    private function fail(CataasContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CataasContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

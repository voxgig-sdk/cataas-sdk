...
class CataasMakeError
{
    public static function call(?CataasContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new CataasContext([], null);
        }
        $op = $ctx->op ?? new CataasOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new CataasResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof CataasError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new CataasError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof CataasError) {

...
import (
	...
	omni "github.com/voxgig-sdk/cataas-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CataasError); is {
				out := map[string]any{
					"name":    "CataasError",
...
func omniCtx(arg any, client *sdk.CataasSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CataasSDK); is {
			client = live

...
{
    private static function utility(): CataasUtility
    {
        return new CataasUtility();
    }
    ...
    private static function ctx(array $over = []): CataasContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new CataasContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new CataasOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof CataasError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new CataasOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = CataasMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new CataasOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = CataasMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new CataasOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = CataasMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CataasSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = CataasMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new CataasResponse([]),
            'result' => new CataasResult([]),
        ]);
        [, $err] = CataasMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new CataasSpec(['step' => 's']),
            'response' => new CataasResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new CataasResult([]),
        ]);
        [, $err] = CataasMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new CataasSpec(['step' => 's']),
            'response' => new CataasResponse(self::resp(200, ['v' => 1])),
            'result' => new CataasResult([]),
        ]);
        [, $err] = CataasMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new CataasSpec(['step' => 's']),
            'response' => new CataasResponse(self::resp(200, ['v' => 2])),
            'result' => new CataasResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        CataasMakeResponse::call($ctx);
    ...
    {
        $preset = new CataasResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new CataasSpec([]),
            'response' => new CataasResponse([]),
            'result' => new CataasResult([]),
        ...
        [$got, $err] = CataasMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new CataasResult([])]);
        [, $err] = CataasMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new CataasSpec([]), 'result' => null]);
        [, $err] = CataasMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CataasSpec(['step' => 's']),
            'result' => new CataasResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new CataasOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CataasMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CataasSpec(['step' => 's']),
            'result' => new CataasResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new CataasOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CataasMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CataasResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new CataasSpec([]),
            'result' => new CataasResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = CataasMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CataasMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new CataasError('boom', 'boom');
        $utility->fetcher = function (CataasContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CataasSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = CataasMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (CataasContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = CataasMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(CataasResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (CataasContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CataasSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = CataasMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new CataasResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new CataasSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = CataasMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = CataasMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = CataasMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new CataasResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, CataasDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new CataasResult(['ok' => false])]);
        $this->expectException(CataasError::class);
        CataasDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new CataasResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', CataasMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new CataasBaseFeature();
        $b = new CataasBaseFeature();
        CataasFeatureAdd::call($ctx, $a);
        CataasFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): CataasBaseFeature
    {
        $f = new CataasBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        CataasFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): CataasContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new CataasSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CataasPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new CataasResponse(['headers' => null]),
            'result' => new CataasResult([]),
        ]);
        CataasResultHeaders::call($ctx);

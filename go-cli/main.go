// Command Cataas-cli is a CLI + REPL for the Cataas SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CATAAS_APIKEY (api key), CATAAS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CATAAS_APIKEY carries the API key and
	// CATAAS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CATAAS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CATAAS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCataasSDK(opts)

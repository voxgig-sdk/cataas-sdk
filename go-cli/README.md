...
boru-driven command-line client **and** interactive REPL for the Cataas
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/cataas-cli)
make build
...
./cataas-cli list cat
./cataas-cli load 1 cat            # {id:1} shorthand
./cataas-cli load '{id:1}' cat       # explicit match map
./cataas-cli list tag

# 5. Override the API base URL for a single call
CATAAS_BASE=https://api.example.com ./cataas-cli list cat
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/cataas-cli
   ```
...
```sh
./cataas-cli load 1 cat          # scalar shorthand for {id:1}
./cataas-cli load '{id:1}' cat     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `cataas>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/cataas-cli`. |
...
(`./cataas-cli <expr>`) and interactively (the REPL), and expressions compose the

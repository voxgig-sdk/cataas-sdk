...
[MCP](https://modelcontextprotocol.io) server exposing the Cataas SDK as
two agent tools — `cataas_list` and `cataas_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/cataas-mcp)
make build

# 2. Provide credentials via the environment
export CATAAS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user cataas \
  -- /absolute/path/to/cataas-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./cataas-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// cataas_list: first page of records
{ "entity": "cat" }
{ "entity": "cat", "query": { } }

// cataas_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/cataas-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user cataas \
     -- "$PWD"/dist/*/cataas-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `cataas_list` and `cataas_load` tools now appear
   in new sessions. Ask the agent to *"list cat using cataas"*
   and it calls `cataas_list` with `{"entity":"cat"}`.
...
```sh
export CATAAS_APIKEY=sk_live_xxx            # API key
export CATAAS_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `cataas_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `cataas_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CATAAS_APIKEY` | API key sent with every request. |
| `CATAAS_BASE` | Optional override of the API base URL. |
...
```sh
./cataas-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"cataas_load","arguments":{"entity":"cat","query":{"id":1}}}}'
```

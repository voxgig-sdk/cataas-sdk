...
MCP server exposing the Cataas SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `cataas_list` | `entity`, optional `query` map | First page of records as JSON |
| `cataas_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `cataas_list` and
`cataas_load` tools become available in new sessions.

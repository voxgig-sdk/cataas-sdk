...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = CataasOmni.make_runner(TEST_JSON_FILE, CataasSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : CataasOmni.errify(err)
    end

...
//     (\`new CataasSDK(...)\` / \`CataasSDK.test(...)\` become
//     \`CataasSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"cat":{"test01":{"id":"test01"}},"tag":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CataasSDK(...)\` and any
// \`CataasSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CataasSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CataasSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

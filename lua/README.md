...
The Lua SDK for the Cataas API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Cat()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/cataas-sdk/releases)),
...
| `tags` | `table` | Tags associated with the cat |
...
```
lua/
├── cataas_sdk.lua    -- Main SDK module
...
The main module (`cataas_sdk`) exports the SDK constructor

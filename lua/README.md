...
The Lua SDK for the Cataas API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Cat(nil):list(nil, nil)
...
local result, err = client:Cataas(nil):load(
  { id = "test01" }, nil
)

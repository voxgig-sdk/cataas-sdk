...
class CataasTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CataasTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CataasTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CataasTestRunner.getenv("CATAAS_TEST_LIVE")
        override = CataasTestRunner.getenv("CATAAS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CataasTestRunner.getenv(key)
        ...
        explain = CataasTestRunner.getenv("CATAAS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["CATAAS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if CataasTestRunner._test_control is not None:
            return CataasTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CataasTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CataasTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CataasTestRunner.load_test_control()
...
def load_env_local():
    CataasTestRunner.load_env_local()


def env_override(m):
    return CataasTestRunner.env_override(m)


def entity_data(v):
    return CataasTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CataasTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CataasTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CataasTestRunner.load_test_control()


def live_client_options():
    return CataasTestRunner.live_client_options()


def live_delay_ms():
    return CataasTestRunner.live_delay_ms()

...
The Python SDK for the Cataas API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Cat(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.Cataas(None).load(
    {"id": "test01"}, None
)
